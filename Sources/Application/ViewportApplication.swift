...
class ViewportApplication: NSObject, NSApplicationDelegate, NSWindowDelegate {
    ...
    override init() {
        let window = NSWindow(
            contentRect: NSMakeRect(0, 0, CGFloat(windowSize.value.width), CGFloat(windowSize.value.height)),
            styleMask: [
    ...
    open func run() {NSApplication.shared.run()}
