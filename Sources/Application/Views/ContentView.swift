...
struct ContentView : View {
    ...
    var body: some View {
        NavigationSplitView(
            ...
            detail: { LoadFontView(for: contentState) }
        ).toolbar {
            ToolbarItemGroup(placement: .status) {
                SharedDebugFontInputView(enabledDebugLevels: $enabledDebugLevels)
                    .onChange(of: enabledDebugLevels) { self.enabledDebugLevels = enabledDebugLevels }
            }
            
            ToolbarItemGroup(placement: .navigation) {
                SharedFontInputView(fontPath: $fontPath)
                    .onChange(of: fontPath) {
                        do {
